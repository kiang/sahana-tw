...
if (file_exists($global['approot'] . 'conf/sysconf.inc')) {
    ...
    $global['stream_type'] = isset($_GET['stream_type']) ? $_GET['stream_type'] : '';
...
function shn_front_controller() {
    ...
    if ($global['stream_type'] && file_exists($approot . '/inc/lib_st_' . $global['stream_type'] . '.inc')) {
        ...
        if (file_exists($approot . '/mod/' . $module . '/' . $global['stream_type'] . '.inc')) $default_file = $approot . '/mod/' . $module . '/' . $global['stream_type'] . '.inc';
        else $default_file = 'stream.inc';
    } else $default_file = 'main.inc';
    ...
    if ($default_file == 'stream.inc') shn_stream_init($module_file);
    ...
    if ($default_file == 'stream.inc') shn_stream_close($module_function, $output);
    //   }else {
