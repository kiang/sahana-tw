...
class VolunteerView extends View {
    ...
    function displayVolunteer($p_uuid) {
        if ($this->model != null) {
            ...
            if (isset($info['gender'])) {
                if (strcasecmp($info['gender'], 'm') == 0) $info['gender'] = 'Male';
                else $info['gender'] = 'Female';
            }
            // get project(s), if any, to which volunteer is assigned
            $projects = array();
            if (is_array($this->model->proj_id)) foreach($this->model->proj_id as $proj_id) $projects[$proj_id] = $dao->getProjectName($proj_id);
            ...
            */
            ...
            if (isset($info['locations'])) {
                foreach($info['locations'] as $loc_uuid) {
                    if ($loc_uuid != null && $loc_uuid != - 1 && $loc_uuid != 'NULL') $location_info[] = $dao->getLocation($loc_uuid);
                }
            }
    ...
    function listVolunteers($volunteers, $extra_opts = array()) {
        ...
        if (VM_LIST_PICTURES && $showPictures) foreach($volunteers as $key => $vol) if ($img_uuid = $dao->getPictureID($vol->p_uuid)) $volunteers[$key]->pictureID = $img_uuid;
    ...
    function addVolunteer($v = null) {
        ...
        */
        ...
        $this->engine->assign('id_types', $dao->getIdTypes());
        if ($v != null) $this->engine->assign('p_uuid', $v->p_uuid);
        //Id type of select from
        if (count($v->info["ids"]) > 0) $id_type = array_pop(array_keys($v->info["ids"]));
        ...
        $hidden = array();
        if ($v != null) $hidden["p_uuid"] = $v->p_uuid;
        ...
        if ($v != null && $img_uuid = $v->getPictureID()) $this->engine->assign('pictureID', $img_uuid);
    ...
    function displayMailbox($box = 'inbox') {
        ...
        foreach($messages as $key => $value) {
            ...
            if (strlen($messages[$key]['message']) >= 34) $messages[$key]['message'] = substr($messages[$key]['message'], 0, 30) . "...";
            $messages[$key]['bgcolor'] = ($value['checked'] == 0) ? '#FFFFA0' : 'white';
            //get the list of message recipients as well as who the message is from
            if ($value['from_id'] == 'SYS_MSG') $messages[$key]['from'] = _('System Message');
            else $messages[$key]['from'] = $dao->getPersonName($value['from_id']);
            ...
            if (strlen($to_list) > 34) $to_list = substr($to_list, 0, 30) . "...";
    ...
    function displayMessage($p_uuid, $message_id, $box = 'inbox') {
        ...
        if (!empty($message)) {
            ...
            if ($message['from_id'] == 'SYS_MSG') $message['from'] = _('System Message');
            else $message['from'] = $dao->getPersonName($message['from_id']);
            //get the list of message recipients
            $list = $dao->getToList($message['message_id']);
            $to_list = "";
            foreach($list as $p_uuid => $name) $to_list.= "$name, ";
            $to_list = substr($to_list, 0, strlen($to_list) - 2);
            $message['to'] = $to_list;
            //apply special syntax rules to the message
            $find = array("/</", "/>/", "/{proj\s+(.+?)\s+(.+?)}/", "/{vol\s+(.+?)\s+(.+?)}/", "/\r\n/", "/\r/", "/\n/");
            $replace = array("&lt;", "&gt;", "<a href=\"?mod=vm&amp;act=project&amp;vm_action=display_single&amp;proj_id=$1\">$2</a>", "<a href=\"?mod=vm&amp;act=volunteer&amp;vm_action=display_single&amp;p_uuid=$1\">$2</a>", "<br />", "<br />", "<br />",);
    ...
    function displaySearchForm($advanced = false, $assigning = false) {
        ...
        foreach($skill_ids as $sk) {
            if ($_REQUEST["SKILL_$sk"] == 'on') $skills[] = $sk;
        }
    ...
    function displaySearchResults($results, $assigning = false, $proj_id = null, $advanced = false, $just_assigned_vol = false, $positions = null) {
        $extra_opts = array('showPictures' => true, 'showAvailability' => true, 'showLocation' => true, 'showStatus' => true, 'showAffiliation' => true, 'showSkills' => true, 'searchParams' => array('advancedSearch' => $advanced, 'justAssignedVol' => $just_assigned_vol), 'showAssignButton' => $assigning, 'modifyProjId' => $proj_id, 'positions' => $positions);
