    ...
    function controlHandler($getvars) {
        ...
        switch ($vm_action) {
            ...
            case 'display_select_project':
                View::View();
                if ($dao->isSiteManager($_SESSION['user_id']) && !$ac->dataAccessIsAuthorized(array('vm_proj_vol' => 'ru'), false)) $projects = $dao->listProjects($_SESSION['user_id'], true);
                else $projects = $dao->listProjects();
            ...
            case 'display_assign':
                ...
                if ($getvars['proj_id'] == '') {
                    ...
                    if ($dao->isSiteManager($_SESSION['user_id']) && !$ac->dataAccessIsAuthorized(array('vm_vol_position' => 'ru'), false)) $projects = $dao->listProjects($_SESSION['user_id'], true);
                    else $projects = $dao->listProjects();
                    $this->displaySelectProjectForAssignmentForm($projects);
                } else {
            ...
            case 'add_position':
                ...
                if ($getvars['pos_id']) $p = new Position($getvars['pos_id']);
                else $p = new Position();
                if (isset($getvars['proj_id'])) $p->proj_id = $getvars['proj_id'];
                $this->addPosition($p);
                break;
        ...
        function validateAddForm($getvars) {
            ...
            if (!isset($getvars['proj_id'])) $validated = $validated && shn_vm_not_empty($getvars['manager'], SHN_ERR_VM_NO_MGR);
