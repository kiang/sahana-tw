        ...
        switch ($getvars['vm_action']) {
            ...
            case 'process_acl_modifications':
                ...
                foreach($getvars as $name => $value) {
                    ...
                    if ($value == 'on' && preg_match("/table_(\w+|_)_req_(\w)/", $name, $matches)) {
                        ...
                        if (!isset($tables[$table_name])) $tables[$table_name] = $permission;
                        else {
                            $current_permissions = $tables[$table_name];
                            if ($permission == 'c') $tables[$table_name] = 'c' . $current_permissions;
                            else if ($permission == 'r') if (substr_count($tables[$table_name], 'c') > 0) $tables[$table_name] = 'cr' . substr($current_permissions, 1);
                            else $tables[$table_name] = 'r' . $current_permissions;
                            else if ($permission == 'u') if (substr_count($tables[$table_name], 'd') > 0) $tables[$table_name] = substr($current_permissions, 0, strlen($current_permissions) - 1) . 'ud';
                            else $tables[$table_name] = $current_permissions . 'u';
                            else
                            //($permission == 'd')
            ...
            case 'process_audit_acl':
                ...
                if ($getvars['process_action'] != '') {
                    ...
                    //($getvars['process_action'] == 'classify_table')
                    {
                ...
                foreach($files as $act => $file_name) {
                    ...
                    if (empty($current_requests[$act])) unset($current_requests[$act]);
                    fclose($handle);
                }
                ...
                foreach($tables as $table) {
                    if (substr($table, 0, 3) == 'vm_') {
                        if (!$dao->isClassified($table)) $unclassified_tables[] = $table;
                    }
                }
