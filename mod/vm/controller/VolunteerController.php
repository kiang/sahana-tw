...
class VolunteerController extends VolunteerView implements Controller {
    ...
    function controlHandler($getvars) {
        ...
        switch ($getvars['vm_action']) {
            case 'process_add':
                ...
                */
                ...
                foreach($skill_ids as $skill) {
                    if ($getvars["SKILL_$skill"] == 'on') $v_skills[] = $skill;
                }
                ...
                */
                if ($this->validateAddForm($getvars = array_merge($_REQUEST, $_FILES))) {
                    ...
                    if (!empty($p['tmp_name'])) {
                        ...
                        $pic->size = $p['size'];
                        $pic->p_uuid = $v->p_uuid;
                        if ($pic->resize()) $pic->save();
                        else add_error(_("The image file is invalid, or is not of a supported type."));
                    }
            ...
            case 'process_change_pass':
                ...
                if ($this->validateChangePassForm($getvars = $_REQUEST)) {
                    ...
                    } else add_confirmation(_("Your Password has been updated"));
                } else $this->changePass($getvars['p_uuid']);
                break;
            case 'process_delete':
                $v = new Volunteer();
            ...
            case 'process_search':
                ...
                if ($assigning) $assigning_proj = $getvars['proj_id'];
                else $assigning_proj = null;
                $location = '';
                if (shn_vm_location_selected()) $location = shn_location_get_form_submit_loc();
                $skills = array();
                $skill_ids = $dao->getSkillIDs();
                foreach($skill_ids as $sk) if ($getvars["SKILL_$sk"] == 'on') $skills[] = $sk;
                ...
                if (!$assigning) $this->displaySearchForm($advanced, false);
                else $this->displaySearchForm(true, true);
                break;
            case 'display_single':
                $v = new Volunteer($getvars['p_uuid']);
                View::View($v);
            ...
            case 'process_remove_picture':
                ...
                View::View();
                if (empty($getvars['p_uuid'])) $this->addVolunteer();
                else $this->addVolunteer(new Volunteer($getvars['p_uuid']));
                break;
            case 'display_portal':
                View::View(new Volunteer($_SESSION['user_id']));
                $this->displayPortal();
            ...
            case 'process_log_time':
                ...
                else $end = $start + $getvars['numHours'] * 60 * 60;
                if (($e = validateShiftTimes($start, $end)) === VM_OK) {
                    if ($dao->logShift($getvars['p_uuid'], $getvars['pos_id'], $start, $end)) {
                        ...
                        View::View($v);
                        $this->displayVolunteer($getvars['p_uuid']);
                    } else add_error(_("There was a problem logging your time. Please go back and try again."));
                } else {
        ...
        function validateAddForm($getvars) {
            ...
            if ($getvars['hrs_avail_start'] != '') $validated = $validated && shn_vm_valid_time($getvars['hrs_avail_start'], SHN_ERR_VM_BAD_START_TIME);
            if ($getvars['hrs_avail_end'] != '') $validated = $validated && shn_vm_valid_time($getvars['hrs_avail_end'], SHN_ERR_VM_BAD_END_TIME);
        ...
        function validateSearchForm($getvars) {
            ...
            if ($getvars['start_date'] != '') {
                if (!shn_vm_valid_date($getvars['start_date'])) {
                    ...
                    $validated = false;
                    $valid_start = false;
                } else $valid_start = true;
            }
            if ($getvars['end_date'] != '') {
                if (!shn_vm_valid_date($getvars['end_date'])) {
                    if ($valid_start !== false) add_error(SHN_ERR_VM_BAD_PROJECT_DATES);
                    $valid_end = false;
                    $validated = false;
                } else $valid_end = true;
            }
            if ($valid_start && $valid_end) $validated = $validated && shn_vm_compatible_dates($getvars['start_date'], $getvars['end_date'], SHN_ERR_VM_DATES_INCOMPATIBLE);
            return $validated;
        }
    }
    /**
