...
class AccessController {
    ...
    public function isAuthorized($error_msg = true, $new_getvars = null) {
        global $dao;
        if (is_null($new_getvars)) $auth_vars = $this->getvars;
        else $auth_vars = $new_getvars;
        ...
        */
        ...
        if (is_array($this->access[$act][$vm_action]['extra'])) {
            foreach($this->access[$act][$vm_action]['extra'] as $value) {
                ...
                if ($value == 'req_login') $passed_vm_acl = $passed_vm_acl && $_SESSION['logged_in'];
                //user must be a volunteer
                if ($value == 'req_volunteer') $passed_vm_acl = $passed_vm_acl && $dao->isVolunteer($_SESSION['user_id']);
                //user must be an approved site manager
                if ($value == 'req_manager') $passed_vm_acl = $passed_vm_acl && $dao->isSiteManager($_SESSION['user_id']);
                //override if user is an approved site manager
                if ($value == 'ovr_manager') if ($dao->isSiteManager($_SESSION['user_id'])) return true;
                //override if user is requesting access to his own information
                if ($value == 'ovr_my_info') if ($auth_vars['p_uuid'] == $_SESSION['user_id']) return true;
                //override if user is requesting access to one of his own project's information
                if ($value == 'ovr_my_proj') if (in_array($auth_vars['proj_id'], array_keys($dao->listProjects($_SESSION['user_id'])))) return true;
                //override if the user is requesting access to one of the projects he is a site manager for
                if ($value == 'ovr_mgr_proj') {
                    if ($dao->isSiteManagerForProject($_SESSION['user_id'], $auth_vars['proj_id'])) return true;
                }
                //override if the user has the Main Operations handler role
                if ($value == 'ovr_mainops') if (in_array(MAINOPS, array_keys(_shn_acl_get_roles($_SESSION['user_id'])))) return true;
                ...
                if ($value == 'ovr_mgr_pos') if ($dao->isPositionUnderManager($auth_vars['pos_id'], $_SESSION['user_id'])) return true;
            }
        }
        if ($passed_vm_acl && $passed_sahana_acl) {
            return true;
        } else {
            if ($error_msg) shn_error_display_restricted_access();
            return false;
        }
    }
    /**
    ...
    public function addMenuItem($text, $getvars, $submenu = false) {
        ...
        if ($this->isAuthorized(false, $getvars)) {
            ...
            foreach($getvars as $key => $value) {
                if ($key == 'act') $url_params.= "$value";
                else $url_params.= "&amp;$key=$value";
            }
            if ($submenu) shn_sub_mod_menuitem($url_params, _($text), $global['module']);
            else shn_mod_menuitem($url_params, _($text), $global['module']);
        }
    }
    /**
    ...
    public function buildURLParams($act, $vm_action, $extra_vars = array()) {
        ...
        foreach($extra_vars as $key => $value) $url_params[$key] = $value;
        return $url_params;
    }
}
?>