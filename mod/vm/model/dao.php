...
class DAO {
    ...
    function remove_keys(&$a, $type = MYSQL_NUM) //
    {
        ...
        if ($type == MYSQL_NUM) {
            ...
            $result = array();
            for ($i = 0;$i < count($a) / 2;$i++) $result[$i] = $a[$i];
            $a = $result;
        }
    }
    /**
    ...
    */
    ...
    function getParentLocations($child, &$locs = array()) {
        ...
        if (!$result->EOF) $locs[$child] = $result->fields['name'];
        if ($result->fields['parent_id'] == null) return $locs;
        else return $this->getParentLocations($result->fields['parent_id'], $locs);
    }
    /**
    ...
    function getVolHoursAndRate($p_uuid, $proj_id = null) {
        ...
        $q = "SELECT  proj_id, project_name, status, pos_id, title, payrate
				 ...
				 WHERE   p_uuid = '$p_uuid' ";
        if (!is_null($proj_id)) $q.= " AND proj_id = '$proj_id' ";
        $r = $this->execute($q);
        $info = array();
        while (!$r->EOF) {
            if (!isset($info[$r->fields['proj_id']])) $info[$r->fields['proj_id']] = array('project_name' => $r->fields['project_name']);
            $info[$r->fields['proj_id']][$r->fields['pos_id']] = array('title' => $r->fields['title'], 'payrate' => $r->fields['payrate'], 'status' => $r->fields['status']);
            $r->MoveNext();
        }
    ...
    function getVolunteers($proj_id = null, $assigned = VM_SHOW_ALL_VOLUNTEERS) {
        if (is_null($proj_id)) {
            if ($assigned == VM_SHOW_ALL_VOLUNTEERS_ASSIGNED) $whereClause = " WHERE vm_vol_active.p_uuid IN (SELECT DISTINCT p_uuid FROM vm_vol_assignment_active)";
            else if ($assigned == VM_SHOW_ALL_VOLUNTEERS_UNASSIGNED) $whereClause = " WHERE vm_vol_active.p_uuid NOT IN (SELECT DISTINCT p_uuid FROM vm_vol_assignment_active)";
            else $whereClause = "  ";
        } else {
    ...
    function getVolunteersForReport($proj_id = null, $org_id = null, $vols = array()) {
        ...
        if (!empty($vols)) {
            ...
            foreach($vols as $index => $p_uuid) {
                $id_list.= "'$p_uuid'";
                if ($index < count($vols) - 1) $id_list.= ", ";
            }
            $id_list.= ")";
            $whereClause.= " AND vm_vol_details.p_uuid IN $id_list ";
        } else {
            if (!is_null($proj_id)) $whereClause.= " AND vm_vol_details.p_uuid IN (SELECT p_uuid FROM vm_vol_assignment WHERE proj_id='$proj_id')";
            if (!is_null($org_id)) $whereClause.= " AND org_id = '$org_id' ";
        }
        $q = "SELECT	DISTINCT person_uuid.p_uuid, person_uuid.full_name, vm_vol_details.status, location_id, org_main.name org_name
				 FROM 		vm_vol_details LEFT JOIN person_uuid ON (person_uuid.p_uuid=vm_vol_details.p_uuid)
							...
							LEFT JOIN org_main ON (vm_vol_details.org_id = org_main.o_uuid) " . $whereClause;
        $result = $this->getCurrentPage($q);
        // put all information into an array for returning
        $volunteers = array();
        while (is_object($result) && !$result->EOF) {
    ...
    function saveVol(&$v, $shn_user = false) {
        if (isset($v->p_uuid) && !$shn_user) {
            ...
            foreach($v->info['ids'] as $id_type => $serial) {
                if (trim($serial) != '') $this->execute("INSERT INTO identity_to_person (opt_id_type,serial, p_uuid) values ('$id_type','$serial'  ,'{$v->p_uuid}')");
                else unset($v->info['ids'][$id_type]);
            }
            ...
            if ($result->EOF) $this->execute("INSERT INTO person_details (p_uuid,opt_gender,birth_date,occupation) values ('" . $v->p_uuid . "','" . $v->info['gender'] . "','" . $v->info['dob'] . "','" . $v->info['occupation'] . "')");
            else $this->execute("UPDATE person_details SET opt_gender = '{$v->info['gender']}', birth_date = '{$v->info['dob']}', occupation = '{$v->info['occupation']}' WHERE p_uuid ='{$v->p_uuid}'");
            ...
            if ($specific_loc == - 1 || $specific_loc == null || $specific_loc === '0') {
                ...
                if ($result->EOF) $this->execute("INSERT INTO location_details (poc_uuid, location_id) VALUES ('{$v->p_uuid}', '$specific_loc')");
                else $this->execute("UPDATE location_details SET location_id='$specific_loc' WHERE poc_uuid='{$v->p_uuid}'");
            }
            ...
            if (!empty($v->info['ids'])) {
                ...
                if ($serial != '') $this->execute("INSERT INTO identity_to_person (opt_id_type,serial, p_uuid) values ('$id_type','$serial'  ,'{$v->p_uuid}')");
                else unset($v->info['ids'][$id_type]);
            }
            ...
            if (!empty($v->info['contact'])) foreach($v->info['contact'] as $key => $value) {
                if (trim($value != '')) $this->execute("insert into contact (pgoc_uuid, opt_contact_type, contact_value) values ('{$v->p_uuid}', '$key', '$value')");
            }
            //insert skill information
            if (!empty($v->info['skills'])) foreach($v->info['skills'] as $skill) {
                $this->execute("INSERT INTO vm_vol_skills (p_uuid, opt_skill_code) VALUES('{$v->p_uuid}', '$skill')");
            }
            //insert the location information
            $specific_loc = $v->info['locations'][0];
            if ($specific_loc != null && $specific_loc != - 1) $this->execute("INSERT INTO location_details (poc_uuid, location_id) VALUES ('{$v->p_uuid}', '$specific_loc')");
        }
    }
    /*
    ...
    */
    function listPositions($proj_id = null, $p_uuid = null) {
        if ($proj_id == null && $p_uuid == null) $whereClause = '';
        else if ($proj_id == null) $whereClause = "WHERE p_uuid = '$p_uuid' AND status='active'";
        else if ($p_uuid == null) $whereClause = "WHERE proj_id = '$proj_id'";
    ...
    */
    ...
    function saveProject(&$p) {
        if (!isset($p->proj_id)) {
            ...
            if ($specific_loc == - 1 || $specific_loc === "0" || $specific_loc === null) $specific_loc = "null";
            else $specific_loc = "'" . $specific_loc . "'";
    ...
    function volIsAssignedToProject($p_uuid, $proj_id = null) {
        ...
        if ($proj_id != null) $query.= " AND proj_id = '$proj_id'";
        $result = $this->execute($query);
        return !$result->EOF;
    }
    /**
    ...
    function getSelectSkillsTree($p_uuid = null, $vol = true, $any_skills = null) {
        ...
        if ($p_uuid != null) $skills_array = $this->getVolSkillsArray($p_uuid);
        else if (is_array($any_skills)) $skills_array = $any_skills;
        else $skills_array = array();
            ...
            foreach($split as $index => $name) {
                ...
                if ($name != '') {
                    if ($index < (count($split) - 1)) {
                        ...
                        if (in_array($result->fields['option_code'], $skills_array)) $extra_info['checked'] = true;
    ...
    function getVolunteersByAbility($ability, $status = null) {
        if ($status != null) $extra_clause = " AND vm_vol_skills.status = '$status' ";
        else $extra_clause = "";
    ...
    function updateAbilityStatus($p_uuid, $ability, $approved) {
        ...
        if (!$result->EOF) {
            if ($approved) {
                ...
                if (!$result->EOF) $this->sendMessage('SYS_MSG', array($p_uuid), _("Your ability status for") . " $ability_name " . _("has been revoked") . ". \n\n (" . _("This is an automated message") . ")");
                else $this->sendMessage('SYS_MSG', array($p_uuid), _("Your ability status request for") . " $ability_name " . _("has been denied") . ". \n\n (" . _("This is an automated message") . ")");
    ...
    function getVolSearchResults($id, $name, $skills, $skills_matching, $start_date, $end_date, $location, $date_constraint, $unassigned = false, $loose = false, $soundslike = false, $assigning_proj = null) {
        ...
        if ($skills_set) {
            ...
            if ($skills_matching == VM_SKILLS_ALL) $query.= "= " . count($skills) . " ";
            else $query.= "> 0 ";
        }
    ...
    */
    ...
    function getPictureID($p_uuid) {
        ...
        if ($result->EOF) return false;
        else return $result->fields['img_uuid'];
    }
    /*
    ...
    */
    ...
    function getAccessRequestConstraints() {
        ...
        while (is_object($result) && !$result->EOF) {
            ...
            if (!is_array($access[$act])) $access[$act] = array();
            if (!is_array($access[$act][$vm_action])) $access[$act][$vm_action] = array();
            if (!is_array($access[$act][$vm_action]['extra'])) $access[$act][$vm_action]['extra'] = array();
        ...
        while (is_object($result) && !$result->EOF) {
            ...
            if (!is_array($access[$act])) $access[$act] = array();
            if (!is_array($access[$act][$vm_action])) $access[$act][$vm_action] = array();
            if (!is_array($access[$act][$vm_action]['tables'])) $access[$act][$vm_action]['tables'] = array();
    ...
    function getAccessRequestName($act, $vm_action) {
        ...
        if (!$result->EOF) return $result->fields['description'];
        else return null;
    }
    /**
    ...
    function getAccessRequests() {
        ...
        while (is_object($result) && !$result->EOF) {
            ...
            if (!is_array($requests[$act])) $requests[$act] = array();
    ...
    function updateClassificationConstraints($act, $vm_action, $tables) {
        ...
        foreach($tables as $table_name => $crud) $this->execute("INSERT INTO vm_access_classification_to_request (request_id, table_name, crud) VALUES ($request_id, '$table_name', '$crud')");
    }
    /**
