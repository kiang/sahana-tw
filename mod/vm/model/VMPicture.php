...
class VMPicture extends Model {
    ...
    function VMPicture($img_uuid = null) {
        if ($img_uuid) {
            ...
            list($this->image_data, $this->thumb_data, $this->p_uuid, $this->width, $this->height, $this->thumb_width, $this->thumb_height, $this->type, $this->name) = $this->dao->getVMPicture($img_uuid);
        }
    }
    /**
     * resize()
    ...
    function resize() {
        ...
        if (!($original = @imagecreatefromstring($this->original))) return false;
     ...
     */
    function display() {
        if (empty($this->image_data)) return false;
        else {
    ...
    function displayThumb() {
        if (empty($this->thumb_data)) return false;
        else {
            header("Content-Type: {$this->type}");
