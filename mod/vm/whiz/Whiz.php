...
class Whiz {
    ...
    function Whiz($cache_dir, $template_dir) {
        ...
        if (!file_exists($cache_dir)) if (!mkdir($cache_dir)) {
    ...
    function clear_cache() {
        if (is_dir($this->cache_dir)) if ($dh = opendir($this->cache_dir)) {
            while (($file = readdir($dh)) !== false) if (!is_dir($file)) unlink($this->cache_dir . $file);
            closedir($dh);
        }
    }
}
