...
class daoTest extends PHPUnit_Framework_TestCase {
    ...
    public function testGetVol() {
        $test1 = array('full_name' => 'Volunteer1', 'ids' => array('dln' => 'serial'), 'locations' => array('489slc-3', '489slc-2', '489slc-1'), 'proj_id' => array(), 'gender' => null, 'dob' => '1986-07-25', 'occupation' => null, 'status' => 'Unassigned', 'date_start' => '2007-09-10', 'date_end' => '2007-10-28', 'hour_start' => '06:00:00', 'hour_end' => '04:00:00', 'affiliation' => 8, 'contact' => array('curr' => '865545565', 'emai' => 'email@star.com'), 'messages' => '1');
        $test2 = array('full_name' => 'Volunteer2', 'ids' => array('dln' => 'cereal'), 'locations' => array('489slc-3', '489slc-2', '489slc-1'), 'proj_id' => array(1000), 'gender' => null, 'dob' => '1982-09-02', 'occupation' => null, 'status' => 'Assigned', 'date_start' => null, 'date_end' => null, 'hour_start' => null, 'hour_end' => null, 'affiliation' => null, 'contact' => array('curr' => '765444465', 'pmob' => '787878788'), 'messages' => '0');
    ...
    private function volResultsAreAccurate($results, $expected_p_uuids, $search = null) {
        foreach($results as $p_uuid => $entry) {
            if (!$this->volResultEntryIsAccurate($entry, $p_uuid, $search)) return false;
        }
    ...
    public function testRemoveArrayKeys() {
        $test = array(0 => 'John Doe', 'name' => 'John Doe', 1 => '12-21-1986', 'birthday' => '12-21-1986', 2 => 'John Doe', 'name1' => 'John Doe', 3 => '12-21-1986', 'birthday1' => '12-21-1986',);
        $this->fixture->remove_keys($test);
        $expected = array('name' => 'John Doe', 'birthday' => '12-21-1986', 'name1' => 'John Doe', 'birthday1' => '12-21-1986',);
    ...
    private function orgResultsAreAccurate($results, $expected_org_ids, $search_results = false) {
        foreach($results as $o_uuid => $entry) {
            if (!$this->orgResultEntryIsAccurate($entry, $o_uuid, $search_results)) return false;
        }
    ...
    private function projResultsAreAccurate($results, $expected_proj_ids, $search_results = false) {
        foreach($results as $proj_id => $entry) {
            if (!$this->projResultEntryIsAccurate($entry, $proj_id, $search_results)) return false;
        }
    ...
    public function testGetSkillIDs() {
        ...
        } else $this->assertTrue(empty($result));
    }
    ...
    public function testGetVMPicture() {
        ...
        $expected = array(md5(file_get_contents('ant_pic.jpg')), md5(file_get_contents('ant_thumb.jpg')), 'ant_guy', 320, 240, 160, 120, 'image/jpeg', 'ant.jpeg');
        $this->assertEquals($result, $expected);
    ...
    public function testSaveVMPicture() {
        ...
        'fbfa2f68b52b8ccd8d24b7b31c90d7d4', 'e4687afe05b6d5c4539b9f1b3b92f95c', 'save_test_person', 320, 240, 160, 120, 'image/jpeg', 'ant.jpg');
    ...
    public function testGetAccessRequests() {
        $count = 0;
        foreach($this->fixture->getAccessRequests() as $act => $info) foreach($info as $vm_action => $desc) {
    ...
    public function testGetSpecificAccessRequestConstraints() {
        ...
        $expected = array('extra' => array('constraint1'), 'tables' => array());
        $this->assertEquals($result, $expected);
    ...
    public function testGetAccessRequestConstraints() {
        $count = 0;
        foreach($this->fixture->getAccessRequestConstraints() as $act => $info) foreach($info as $vm_action => $info) {
            if (isset($info['extra'])) {
    ...
    public function testGetSkillsAndDescriptions() {
        ...
        $expected = array('WAR1' => 'Unskilled-Warehouse-Forklift',);
        $this->assertEquals($result, $expected);
