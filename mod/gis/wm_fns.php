...
function show_wiki_map($category = "all", $date = 0) {
    ...
    shn_form_fclose();
    $db = $global['db'];
    $query = "select a.wiki_uuid,a.name,a.description,a.url,date(a.event_date) as event_date,a.author,a.editable,b.map_northing,b.map_easting from gis_wiki as a, gis_location as b where a.wiki_uuid=b.poc_uuid " . (($category == 'all') ? " " : " and opt_category='{$category}'");
    ...
    while (!$res->EOF) {
        ...
        array_push($map_array, array("lat" => $res->fields['map_northing'], "lon" => $res->fields['map_easting'], "name" => $res->fields['name'], "desc" => $res->fields['description'], "url" => $res->fields['url'], "author" => $res->fields['author'], "edit" => $edit_url, "date" => $res->fields['event_date'], "wiki_id" => $res->fields['wiki_uuid'], "image" => $image_path, "img_w" => 100, "img_h" => 100));
        $res->MoveNext();
    }
    //call gis api
...
?>
	<h2><?php echo _("Mapping Client") ?></h2>
	<p></p>
<?php
    //mapping comes here
    $options = array();
...
function show_wiki_add_detail($errors = false) {
    ...
    include_once $global['approot'] . "/inc/lib_form.inc";
?>
	<h2><?php echo _("Add Situation Detail") ?></h2>
<?php
    shn_form_fopen(awik);
