    ...
    function fetch($URI) {
        ...
        function submit($URI, $formvars = "", $formfiles = "") {
            ...
            function fetchform($URI) {
                if ($this->fetch($URI)) {
                    if (is_array($this->results)) {
                        for ($x = 0;$x < count($this->results);$x++) $this->results[$x] = $this->_stripform($this->results[$x]);
            ...
            function _httpsrequest($url, $URI, $http_method, $content_type = "", $body = "") {
                ...
                //$headers[] = $http_method." ".$url." ".$this->_httpversion;
                if (!empty($this->agent)) $headers[] = "User-Agent: " . $this->agent;
                if (!empty($this->host)) if (!empty($this->port)) $headers[] = "Host: " . $this->host . ":" . $this->port;
                ...
                for ($currentHeader = 0;$currentHeader < count($result_headers);$currentHeader++) {
                    ...
                    if (preg_match("|^HTTP/|", $result_headers[$currentHeader])) $this->response_code = $result_headers[$currentHeader];
